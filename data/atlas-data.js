...
export const CO= {
  ...
  huawei:{n:'Huawei / HiSilicon',tk:'PRIVATE',ctry:'China',slug:'',role:'Ascend AI accelerators. Roadmap: 910C (2025) → 950PR/950DT (2026) → 960 (2027) → 970 (2028). From the 950 series it uses in-house HBM: HiBL 1.0 (128 GB, 1.6 TB/s) and HiZQ 2.0 (144 GB, 4 TB/s). Today\'s 910C still runs on stockpiled Samsung/SK Hynix HBM2E. HBM supply, not compute, gates its volume.',makes:'Ascend accelerators, in-house HBM',mx:['HiBL/HiZQ volume','SMIC allocation','domestic purchase mandates']},
...
export const HIER=[
  ...
  {id:'hbm',name:'HBM3E / HBM4',kind:'On-package DRAM · the prize',col:'#5eead4',cap:'80–288 GB',bw:'3.35–8 TB/s',lat:'150–220 ns',en:'~4 pJ/b',sev:5,
   why:'Only three companies on earth can make HBM (SK Hynix, Samsung, Micron) — and even their output is capped from outside: every stack must be bonded to the GPU on TSMC\'s CoWoS packaging line (sold out into 2027) and its leading DRAM dies are printed on ASML\'s EUV machines (a literal monopoly). All three are sold out through 2027 and filling only 50–70% of what customers ask for, so <b>allocation</b>, not price, decides who ships GPUs.',
   ...
   biz:'<span class="hl">The center of gravity.</span> A 3-way oligopoly (<b>SK Hynix</b>, <b>Samsung</b>, <b>Micron</b>) makes the stacks — all three are now qualified on NVIDIA\'s Rubin HBM4, with SK Hynix holding ~60–70% of that allocation. <b>TSMC</b> CoWoS packaging is the real ceiling on supply (and now makes SK Hynix\'s base die too); <b>Besi</b>/<b>ASML</b>/Japan materials gate it from below. HBM is ~5–10× the cost/GB of DDR, is sold out through 2027, and is eating the DRAM wafers commodity memory used to get — <span class="hl2">see The DRAM Cycle</span>.',
   cos:['skhynix','samsung','micron','tsmc','besi','asml','lam','disco','ase','ajinomoto','ibiden','shinetsu'],
   geo:[['KOR','HBM stacks — SK Hynix, Samsung (~80% of the world\'s HBM, from two cities)'],['TWN','CoWoS packaging — TSMC (sold out to 2027)'],['NLD','EUV litho — ASML (100% monopoly); bonding — Besi'],['JPN','wafers/resist/ABF — Shin-Etsu, Ajinomoto, Ibiden'],['USA','Micron HBM, Lam tools, GPU design']],
   share:{title:'HBM market share, 2025 (revenue)',seg:[['SK Hynix',62,'#5eead4'],['Micron',21,'#46c8e0'],['Samsung',17,'#5aa9ff']]},
...
export const EXTRA={
  cxl_switch:{id:'cxl_switch',name:'The Interconnect War',kind:'CXL · NVLink · UALink',col:'#9a8cff',sev:4,
    ...
    blurb:'Inside a rack, GPUs must talk to each other and to pooled memory at terabytes per second. Three standards fight for it. NVIDIA\'s proprietary <b>NVLink</b> is shipping at 1.8 TB/s — the moat. The open <b>UALink</b> consortium (AMD, Broadcom, Intel, Google, Meta) published its 2.0 spec in April 2026 <em>before</em> any 1.0 silicon shipped; AMD\'s MI400 carries the first in H2 2026, with broad products in 2027. <b>CXL</b> pools coherent memory.',
  ...
  phot_transceiver:{id:'phot_transceiver',name:'Optical Transceivers',kind:'800G / 1.6T modules',col:'#7ee787',sev:3,
    why:'Assembly is abundant (Chinese makers fill 7 of the top-10 slots and ~60% of NVIDIA\'s 800G volume) — but every single module needs a US-designed DSP chip and a scarce laser inside. One export-control decision or one laser shortage stalls the whole segment.',
  ...
  hbm_basedie:{id:'hbm_basedie',name:'The HBM4 Base Die',kind:'where memory becomes logic',col:'#9a8cff',sev:4,
    ...
    stats:[['HBM4 bus','2,048-bit'],['SK Hynix base die','TSMC 12nm · 3nm evaluated for HBM4E'],['Samsung base die','in-house 4nm'],['Rubin allocation','~65 / 27 / rest']],
    ...
    mx:[['Rubin HBM4 allocation','SK Hynix ~60–70% · Samsung ~25–30% · Micron the rest — the scoreboard of the two approaches'],['HBM4E base-die node','TSMC 3nm confirmation = the moat deepens'],['Custom HBM wins','hyperscaler-specific base dies = the next design-win war'],['16-Hi qualification','Q4 2026 samples to NVIDIA']],
  ...
  china:{id:'china',name:'China\'s Stack',kind:'what it can and cannot make',col:'#ff5d6c',sev:4,
    ...
    blurb:'<b>What China has:</b> CXMT at ~10% of global DRAM revenue and ~300k wafers/month after a 466% IPO pop; InnoLight and Eoptolink at ~60% of NVIDIA\'s 800G modules; Huawei\'s Ascend line with in-house HBM (HiBL/HiZQ) from the 950 series; Montage\'s PCIe 6 / CXL 3 retimer on the PCI-SIG integrators list; JCET\'s XDFOI 2.5D packaging line in production. <b>What it lacks:</b> EUV, HBM3E at scale, EML lasers, optical DSPs — and a leading-edge logic die that isn\'t a pre-ban TSMC stockpile.',
    biz:'For the tickers on this map China is a <b>risk in both directions</b>. Downside: Beijing\'s ban on buying NVIDIA\'s China-specific chips (H20, RTX Pro 6000D; a B30A approval now unlikely) has all but closed the China TAM; the gallium/germanium reprieve to the US expires 27 Nov 2026; and a <em>proposed</em> FCC rule to bar Chinese transceivers from US networks — not enacted, status unverified — would hit <b>InnoLight</b> and <b>Eoptolink</b>\'s core customers if it ever landed. Upside for the incumbents: every year China stays without EUV and HBM3E is another year of pricing power for <b>ASML</b>, <b>SK Hynix</b>, <b>Lumentum</b> and <b>Marvell</b>. Watch the BIS review of the HBM rule (3A090.c) due December 2026 — it is the next scheduled shock.',
...
export const ELI={
  ...
  hbm:'Picture bolting a stack of ultra-fast memory chips <em>directly onto</em> the GPU so data barely has to travel. That\'s HBM. It is the scarcest, most fought-over part in all of AI: only three companies can make it, it costs 5–10× normal memory, and it is sold out through 2027. If you remember one thing from this whole map, make it this.',
  ...
  phot_transceiver:'The little modules that convert electrical signals into light at every switch port. China\'s InnoLight and Eoptolink build about 60% of NVIDIA\'s — but the critical brain inside each one is a US-made chip (Marvell). That dependency is the real point of leverage in the whole optics story.',
  ...
  dram_cycle:'HBM gets the attention, but ordinary memory prices still drive the memory companies\' earnings. Those prices are spiking now, because HBM and ordinary DRAM compete for the same factories and HBM is winning. New factories arrive mainly in 2027–28, so the squeeze could last until then. That is the whole cycle.',
...
export const GEO=[
  ...
  {cc:'KOR',country:'South Korea',sev:5,role:'HBM manufacturing — SK Hynix + Samsung, ~80% of global HBM (Micron is the rest), made in two cities. The memory every AI GPU is built around.',cos:['skhynix','samsung']},
  ...
  {cc:'CHN',country:'China',sev:4,role:'Optical-module volume (InnoLight + Eoptolink, ~60% of NVIDIA\'s 800G) and a rising commodity-DRAM supplier (CXMT ~10% of global revenue) — but no EUV, no HBM3E before 2027, no domestic DSP and <15% of its high-speed lasers. Holds the gallium/germanium/rare-earth inputs the West\'s lasers need, and has banned purchases of NVIDIA\'s China-specific chips. The chokepoint that cuts both ways.',cos:['cxmt','smic','huawei','innolight','eoptolink','montage']},
...
export const SHORTLIST=[
  {t:'Chokepoint monopolies',d:'Single points of failure — if any one stalls, the whole stack stalls. Highest barriers, near-impossible to displace.',items:[
    ['tsmc','Makes the GPU AND the CoWoS packaging that bonds HBM to it — everything routes through its fabs. CoWoS sold out into 2027.'],
...
export const JOURNEY=[
  ...
  {r:'hierarchy',n:'hbm',t:'HBM — the prize',cap:'If you remember one stop, make it this one. Just three firms make HBM and it is sold out through 2027 — gated from below by TSMC\'s CoWoS packaging, ASML\'s EUV monopoly and Ajinomoto\'s 95% grip on ABF. The bottleneck is packaging, not the chip. Tap <b>"Who makes it"</b> in the panel.'},
